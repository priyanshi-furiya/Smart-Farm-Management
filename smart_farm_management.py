...
# conn.commit() is used to save the changes made to the database
            c.execute('''CREATE TABLE farmers (username text, password text, state text, contact text,area text,trust_score text)''')
...
# c.execute is used to execute the sql queries and insert the values into the table
        c.execute("INSERT INTO farmers VALUES (?, ?, ?, ?, ?,?)",(username, password, state, contact, area, trust_score))
        conn.commit()
        txt = "Account created"
        print (txt.center(140, " "))
...
# make connection with the details database and create a table with the given columns
                ...
                if os.path.exists('details_database.db'):
                    ...
                    conn = sqlite3.connect('details_database.db')
                    c = conn.cursor()
                    c.execute('''CREATE TABLE details (username text,crop text, sowing_date text, expected_harvest_date text, expected_yield text, alerts text)''')
                username = user
...
# Add Calendar
                cal = Calendar(root, selectmode = 'day',year = 2023, month = 4,day = 14)
                cal.pack(pady = 20)
                def grad_date():
...
# again creating calender object to accept the expected harvest date of the crop from the farmer
                ...
                print("Enter the sowing date of the crop:",expected_harvest_date)
                # expected_harvest_date = input("Enter the Expected harvest date of the crop: ")
                expected_yield = input("Enter the Expected yield of the crop: ")
                alerts = input("Enter the Alerts related to pests and diseases: ")
                c.execute("INSERT INTO details VALUES (?, ?, ?, ?, ?, ?)", (user,crop, sowing_date, expected_harvest_date, expected_yield, alerts))
                conn.commit()
                # print("Exporting data into CSV............")
                cursor = conn.cursor()
...
# make connection with the customers database and create a table with the given columns 
        if os.path.exists('customers_database.db'):
            ...
            conn = sqlite3.connect('customers_database.db')
            c = conn.cursor()
            c.execute('''CREATE TABLE customers (username text, password text, contact text)''')
        print(" ")
        print("Enter details to create an account")
        username = input("Enter your username: ")
        password = pwinput.pwinput("Enter your password: ", "*")
        contact = input("Enter your contact number: ")
        c.execute("INSERT INTO customers VALUES (?, ?, ?)",(username, password, contact))
        conn.commit()
        txt = "Account created"
        print (txt.center(140, " "))
...
# if the customer selects 2 then filter by state
                ...
                state_demand = input("Enter the state where you need the crop: ")
                data = pd.read_csv("details_database.csv")
                data1 = pd.read_csv("farmers_database.csv")
...
# once you were able to contact the farmer then you can rate the farmer
        ...
        if choice == 1:
            ...
            if os.path.exists('farmers_database.db'):
                ...
                conn = sqlite3.connect('farmers_database.db')
                c = conn.cursor()
                c.execute('''CREATE TABLE farmers (username text, password text, contact text, area text, state text, trust_score text)''')
            data = pd.read_csv("farmers_database.csv")
            trust = data.loc[data['username'] == name, 'trust_score'].values[0]
            trust_score = (trust_score + trust)/2
            c.execute("UPDATE farmers SET trust_score = ? WHERE username = ?", (trust_score, name))
            conn.commit()
